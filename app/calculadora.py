...
class Calculadora(object):
    """Kelas untuk membuat tata letak kalkulator, mendistribusikan tombol-tombol,
dan menambahkan fungsionalitasnya.

Tombol-tombol yang didistribusikan pada tata letak sesuai dengan contoh di bawah ini:
    ...
    CATATAN: Diperlukan untuk mengimpor modul style yang terdapat dalam paket view,
             dan memilih salah satu kelas gaya yang tersedia.
    """
    ...
    def __init__(self, master):
        ...
        # Edição da Top-Level
        self.master.title('KALKULATOR RIVALDI')
    ...
    def _create_menu(self, master):
        ...
        calc_menu.add_cascade(label='Kalkulator', menu=config)
        config.add_cascade(label='Tema', menu=theme)

        config.add_separator()
        config.add_command(label='Keluar', command=self._exit)
